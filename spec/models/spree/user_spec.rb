...
module Spree
  describe User, :type => :model do
    it { is_expected.to respond_to(:store_credits)}
    ...
    context '#has_store_credit?' do
      it 'should return true for users with credits' do
        expect(user_with_credits.has_store_credit?).to be_truthy
      end

      it 'should return false for users without credits' do
        expect(user_without_credits.has_store_credit?).to be_falsey
      end
    end
